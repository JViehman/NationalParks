...
```{r}
frame = data.frame(npsdat$ParkName, Y, kmeans2.out$cluster)
frame = setNames(frame, c("Name","logVisits","Cluster"))
head(frame)
```
...
cluster4 = frame[frame$Cluster == 4,]
cluster5 = frame[frame$Cluster == 5,]
```
...
```{r}
unique(cluster4$Name)
length(unique(cluster4$Name))
```

```{r}
unique(cluster5$Name)
length(unique(cluster5$Name))
```